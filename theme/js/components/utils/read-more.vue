...
<template>
  <div ref="container" class="read-more" :class="{expand: expanded}" :style="{height: containerHeight + 'px'}">
    <slot></slot>
    <div class="read-more__bottom" @click.prevent="toggle">
      <button role="button" class="fr-btn fr-btn--tertiary-no-outline" @click.prevent.stop="toggle" v-if="readMoreRequired">
