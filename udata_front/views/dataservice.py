...
def recent_feed():
    ...
    for dataservice in dataservices:
        ...
        if dataservice.organization:
            ...
            author_uri = url_for(
                "organizations.show", org=dataservice.organization.id, _external=True
            )
        elif dataservice.owner:
            author_name = dataservice.owner.fullname
            author_uri = url_for(
                "users.show", user=dataservice.owner.id, _external=True
            )
        feed.add_item(
            dataservice.title,
            description=dataservice.description,
            content=render_template(
                "dataservice/feed_item.html", dataservice=dataservice
            ),
            author_name=author_name,
            author_link=author_uri,
            link=url_for(
                "dataservices.show", dataservice=dataservice.id, _external=True
            ),
